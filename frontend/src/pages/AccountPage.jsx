...
export default function AccountPage() {
    ...
    return (
        <div className="page-container">
                            ...
                            <Row label="OS ID" border={true}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text-1)', background: 'var(--bg-surface)', padding: '4px 8px', borderRadius: '6px', border: '1px solid var(--border)', letterSpacing: '0.05em' }}>
                                        {profile?.os_id || profile?.username || ('GEN-' + (profile?.id || '0000').substring(0,4).toUpperCase())}
                                    </span>
                                </div>
                            </Row>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                <Section title="Active Sessions">
                                    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', height: '100%' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', border: '1px solid var(--border)', borderRadius: '12px', background: 'var(--bg-elevated)', flex: 1 }}>
            ...
            <Modal isOpen={showPricingModal} onClose={() => setShowPricingModal(false)} maxWidth="440px">
