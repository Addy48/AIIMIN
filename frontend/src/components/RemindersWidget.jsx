    ...
    const handleSave = async () => {
        ...
        try {
            if (mode === 'note') {
                ...
                const noteText = `[Quick Note]: ${text.trim()}`;
                ...
                await supabase
                    .from('daily_logs')
                    .upsert({ date: today, journal_entry: newJournal }, { onConflict: 'user_id,date' });

            } else if (mode === 'reminder') {
                // For MVP, save reminder to localStorage to simulate local push notifications / sticky reminder
                const newReminder = {
                    id: Date.now(),
                    text: text.trim(),
                    time: reminderTime,
                ...
                const existing = JSON.parse(localStorage.getItem('aiimin_reminders') || '[]');
                localStorage.setItem('aiimin_reminders', JSON.stringify([...existing, newReminder]));
            }
