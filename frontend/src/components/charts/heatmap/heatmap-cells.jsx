...
function syncHeatmapCellLayerOpacities(params) {
  ...
  if (!animateCells) {
    if (isReadyResting) {
      ...
      dataOpacity.set(0);
      pulseOpacity.set(0);
      shimmerOpacity.set(participates && showLoadingCellsLayer
        ? 0
        : HEATMAP_LOADING_BASE_CELL_OPACITY);
    }
    return;
...
const HeatmapMotionCell = memo(function HeatmapMotionCell({
  ...
  const participates = useMemo(() =>
    heatmapLoadingCellParticipates(cell.column, cell.row, loadingCellRandomness), [cell.column, cell.row, loadingCellRandomness]);
...
export const HeatmapCells = memo(function HeatmapCells({
  ...
  const handleCellEnter = useCallback((column, row, bin, x, y) => {
    if (!cellsInteractive) {
      return;
    }
