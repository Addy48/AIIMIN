...
await test('NVIDIA_API_KEY (KIMI)', async () => {
  ...
  if (!key) return { ok: false, status: 0, note: 'missing' };
  const model = process.env.NVIDIA_CHAT_MODEL || 'meta/llama-3.2-11b-vision-instruct';
...
await test('OPENROUTER_API_KEY', async () => {
  ...
  if (!key) return { ok: false, status: 0, note: 'missing' };
  const model = process.env.OPENROUTER_LITE_MODELS?.split(',')[0] || process.env.OPENROUTER_MODEL || 'nex-agi/nex-n2.5-mini:free';
