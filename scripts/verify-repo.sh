...
if [ -d app ]; then
  ...
  if [ -n "$NATIVE_JAVA" ]; then
    echo "Building app debug (JDK 17)..."
    if (cd app && JAVA_HOME="$NATIVE_JAVA" ./gradlew :app:assembleDebug --no-daemon -q >/tmp/aiimin-verify-native.log 2>&1); then
      pass "app assembleDebug"
    else
      fail "app build failed (see /tmp/aiimin-verify-native.log)"
    fi
  else
...
if git ls-files 'app/**/build/**' 'frontend/android/**/build/**' 2>/dev/null | grep -q .; then
