...
**Journal.** Modes: Today, Free Write, CBT record, What Went Well, Morning Pages, Weekly Review. Mode is a **chip after entry starts**, never a gate before writing (kill list: "journal mode gate"). Journal body is **private reflection** — excluded from analytics/telemetry (P8-R-219); AI touches it only for the user's own explicit request (Journal AI), and prompts send **minimum necessary** text (§11.6).
...
| Tier | Price (list / founding) | Key unlocks | AI calls/day |
|------|------------------------|-------------|--------------|
| **Explore** | Free | Today, Depth, capture, journal, notes, calendar, account, reports (snapshot locked), **English: 1 drill/day** | 1 |
| **Core** | ₹29 / complimentary at launch for waitlist | Habits, goals, finance, focus, lab (**full English**), sports, discipline, career, Snapshot report, journal AI | 10 |
| **Pro** | ₹59 / ₹49 | **Family + vault**, PDF Life OS Review (6/mo), patterns/correlations, wealth AI, what-if, cloud voice replay, certificate export | 25 |
| **Elite** | ₹99 / ₹79 | Interactive Intelligence report, 3 deep reports/mo (separate pool), unlimited standard PDFs | 40 |

Rules: show **all four** tiers, never three · founding strikethrough on Pro/Elite · "Recommended" only on Pro · plan chip `Pro · till 10 Aug 2026` · upgrade celebration is a 3-beat animation, not confetti spam · **no purple premium styling** · caps are stated honestly (never "unlimited AI") · downgrade never deletes data — gated features become read-only with an export path · backend is the entitlement authority (SYS-02); Play/App Store purchases reconcile server-side.
**Edge cases.** Payment failed (grace 7 days, banner not lockout) · refund/chargeback (revert tier, keep data) · store purchase on a different account (link flow with explicit confirmation) · tier change mid-billing (prorate rules documented in Terms) · offline purchase attempt (queued, honest message) · quota exhausted mid-action (the action completes if already started; the next one is blocked with the reset time).
...
| OD-09 | iOS timing | (a) after Android V1 (b) parallel | **(a)** — Screen Time parity is impossible on iOS; avoid a broken promise | POST-V1 |
...
*End of Blueprint v1.0. This document is living: append changes with dates; do not silently rewrite decisions. It expresses Genesis and cannot amend it.*
