...
```
backend/ (or server/)
├── supabase_init.sql   ← single schema source
├── index.js
├── routes/             → 13+ route files
├── controllers/
├── middleware/         → auth, sanitize, rate limit
├── jobs/               → drift, commitment evaluators
└── lib/                → cache, googleClient, aiVoice
```

## Notable routes

auth, dailyLogs, calendar, habits, finance, family, waitlist, intelligence, cron
