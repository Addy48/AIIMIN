...
fun FamilyRoute(
    ...
    FamilyScreen(state = state, onBack = onBack, modifier = modifier)
}

/** One job: shared care list. Files stay on web. */
@Composable
fun FamilyScreen(
    ...
    Column(
        ...
        ScreenHead(title = "Family", meta = state.familyMeta)
        if (state.family.isEmpty()) {
            Text(
                text = "No family documents on this graph yet.",
                style = AiiminTheme.type.bodySmall,
                color = AiiminTheme.colors.muted,
                modifier = Modifier.padding(top = AiiminTheme.space.s3),
            )
        } else {
            SectionRule(label = "Documents")
            state.family.forEachIndexed { i, row ->
                Text(
                    text = row.title,
                    style = AiiminTheme.type.body,
                    modifier = Modifier.padding(vertical = 10.dp),
                )
                if (i < state.family.lastIndex) HairRule()
            }
        }
        TapSurface(
            onClick = {
