...
export async function getAnalyticsDataset(userId, windowDays = 120) {
    const cacheKey = `analytics-dataset:${userId}:${windowDays}`;
    const cached = cacheGet(cacheKey);
    if (cached) return cached;

    const sinceDate = new Date(Date.now() - ((windowDays - 1) * DAY_MS)).toISOString().slice(0, 10);
    ...
    const [
        ...
        pool.query(
            `SELECT DATE(started_at AT TIME ZONE 'UTC') AS date,
             ...
             WHERE user_id = $1 AND DATE(started_at AT TIME ZONE 'UTC') >= $2
             GROUP BY 1
             ORDER BY 1 ASC`,
            [userId, sinceDate]
        ),
        ...
        pool.query(
            `SELECT DATE(started_at AT TIME ZONE 'Asia/Kolkata') AS date,
             ...
             FROM routine_runs
             WHERE user_id = $1 AND DATE(started_at AT TIME ZONE 'Asia/Kolkata') >= $2
             GROUP BY 1
             ORDER BY 1 ASC`,
            [userId, sinceDate]
        ),
        pool.query(
            `SELECT DATE(completed_at AT TIME ZONE 'Asia/Kolkata') AS date,
             ...
             FROM habit_logs
             WHERE user_id = $1 AND DATE(completed_at AT TIME ZONE 'Asia/Kolkata') >= $2
             GROUP BY 1
             ORDER BY 1 ASC`,
            [userId, sinceDate]
        ),
